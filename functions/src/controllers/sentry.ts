...
export const postNotificationFromSentryToSlack = async (
    req: Request,
    res: Response,
    next: NextFunction
): Promise<void> => {
  ...
  // OS情報
  const osInfo =
    os === undefined ? "unknown" : `${os.name} ${os.type} ${os.version}`;
  // デバイス情報
  ...
  const userInfo =
    user === undefined ?
      "unknown" :
      `IP:${user.ip_address} address: ${user.geo.region} ${user.geo.city}`;
  // Sentryのissue URL
  const issue = {
    message: title,
    detail: url,
